@extends('layout')

@section('title')Login - Lumina Framework@endsection

@section('content')
<div class="row justify-content-center">
    <div class="col-md-5">
        <div class="card shadow-lg border-0 mt-5">
            <div class="card-body p-5">
                <div class="text-center mb-4">
                    <h2 class="fw-bold text-primary">Welcome Back</h2>
                    <p class="text-muted">Please enter your credentials to login</p>
                </div>
                
                @if(success_msg)
                <div class="alert alert-success">{{ success_msg }}</div>
                @endif
                @if(error_msg)
                <div class="alert alert-danger">{{ error_msg }}</div>
                @endif
                
                <form action="/auth/login" method="POST">
                    @csrf
                    
                    <div class="mb-3">
                        <label class="form-label">Email Address</label>
                        <input type="email" name="email" class="form-control form-control-lg @if(errors and errors.email) is-invalid @endif" 
                            placeholder="name@example.com" value="{{ old.email | default(value="") }}" required>
                        @if(errors and errors.email)
                            <div class="invalid-feedback">{{ errors.email }}</div>
                        @endif
                    </div>
                    
                    <div class="mb-3">
                        <label class="form-label">Password</label>
                        <input type="password" name="password" class="form-control form-control-lg @if(errors and errors.password) is-invalid @endif" 
                            placeholder="Enter password" required>
                        @if(errors and errors.password)
                            <div class="invalid-feedback">{{ errors.password }}</div>
                        @endif
                    </div>
                    
                    <div class="d-grid gap-2 mt-4">
                        <button type="submit" class="btn btn-primary btn-lg rounded-pill">Login Now</button>
                    </div>
                    
                    <div class="text-center mt-4">
                        <p class="mb-0">Don't have an account? <a href="/auth/register" class="text-primary fw-bold text-decoration-none">Register</a></p>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>
@endsection
